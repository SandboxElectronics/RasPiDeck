...
uint8_t SPIPi::transfer(uint8_t value){
    ...
    //printf("SPI transfer step 1\n"); 
    bcm2835_peri_set_bits(paddr, BCM2835_SPI0_CS_CLEAR, BCM2835_SPI0_CS_CLEAR);
    //printf("SPI transfer step 2\n");
    bcm2835_peri_set_bits(paddr, BCM2835_SPI0_CS_TA, BCM2835_SPI0_CS_TA);
    //printf("SPI transfer step 3\n");
    while (!(bcm2835_peri_read(paddr) & BCM2835_SPI0_CS_TXD))
    delayMicroseconds(10);
    //printf("SPI transfer step 4\n");

    bcm2835_peri_write_nb(fifo, value);
    //printf("SPI transfer step 5\n");
    while (!(bcm2835_peri_read_nb(paddr) & BCM2835_SPI0_CS_DONE))
    delayMicroseconds(10);
    //printf("SPI transfer step 6\n");

    uint32_t ret = bcm2835_peri_read_nb(fifo);
    //printf("SPI transfer step 7\n");

    bcm2835_peri_set_bits(paddr, 0, BCM2835_SPI0_CS_TA);
    //printf("SPI transfer step 8\n");
...
void pinMode(int pin, Pinmode mode){
    ...
    if ((pin <= 13)||(pin == PL)) {
        ...
        pin = pin-A0;
        RasPiDeckI2CIOSetPinMode(pin,mode);
    };
     
/*
	else if (pin <= 19) {
	    direction = SerialIO.direction;
        ...
        if (direction != SerialIO.direction) {
            SerialIO.direction = direction;
            reg = SerialIO.direction & SerialIO.state;

            Wire.beginTransmission(SERIAL_IO_ADDR);
            Wire.write(reg);
...
void digitalWrite(int pin, int value) 
{
    ...
    if ((pin <= 13)||(pin==PL)) {
     //   printf("before pin map\n");
        pin = raspberryPinNumber(pin);
       // printf("after pin map\n");
        if (value == HIGH){
            switch(pin){
       ...
       RasPiDeckI2CIOSetPinStatus(pin-A0, value);
    }

/*
    else if (pin <= 19) {
	    state = SerialIO.state;
        ...
        if (state != SerialIO.state) {
            SerialIO.state = state;
            reg = SerialIO.direction & SerialIO.state;
	...
	} else if (pin == PL)  { // Peripheral Transceiver Enable/Disable
                     if (value == HIGH){
                         GPSET0 =  BIT_7;
                     } else {
                        GPCLR0 =  BIT_7;
...
int digitalRead(int pin){
	...
	if ((pin <= 13)||(pin==PL)) {
            ...
            if ( RasPiDeckI2CIOGetPinStatus((uint8_t)pin-A0) == 0) {
                return LOW;
            } else {
                return HIGH;
            }
...
/*
	    ...
	    if (Wire.requestFrom(SERIAL_IO_ADDR, 1)) {
            reg = Wire.read();
            if (reg & (1 << (pin-14))) {
                value = HIGH;
            } else {
...
int analogRead(int channel) {
    ...
    Wire.write((channel_code<<4) + 0x84);
    Wire.endTransmission();

    count = Wire.requestFrom(0x48, 2);    //
	//  printf("Got %d bytes\n", count);

    if (count == 2) {
        adc_result  = (uint8_t)Wire.read();
        adc_result  = adc_result << 8;
        adc_result += (uint8_t)Wire.read();

            //printf("channel_%d (0x%02X) = %d (%.5fV)\n", channel, channel * 0x10 + 0x84, adc_result, adc_result * 5.0 / 4095);
    } else {
	  //printf("Invalid Data\n");
    }
	//    }
...
int main(int argc,char *argv[]) {
  uint8_t i; 
  uint8_t skip_reset=0;

  for (i=1;i<argc;i++) {
    if ( !strcmp(argv[i], "-u") ) {  //is -u option is specified, bypass the reset routine
      printf("!!!!!=====!!!!!=====Reset routine skipped=====!!!!!=====!!!!!\n");
      skip_reset = 1; 
     getchar(); 
     }
  }
